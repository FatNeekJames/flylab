...
function frame(now){const dt=Math.min((now-last)/1000||0,.05);last=now;clock+=dt;if(!life.active&&life.owner)motion.step(engine,dt);
 const activity=life.skill,pose=activityView.update(activity,clock),growth=core.development.growth;sceneDistance=activityView.cameraDistance;sceneHeight=activityView.cameraHeight;view();floor.scale.set(pose?1.5:1,1,pose?1.5:1);host.setAttribute('aria-label',pose?`${activity.label}: ${activityView.status}. Drag to orbit; scroll to zoom.`:'Interactive 3D fly bench press. Drag to orbit; scroll to zoom.');thorax.scale.set(.33*(1+.16*growth),.28*(1+.08*growth),.38);abdomen.scale.x=.44*(1+.06*growth);
