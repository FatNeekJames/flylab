...
const $=id=>document.getElementById(id), engine=new FlyEngine(),motion=new BenchMotion();
...
orb(0,1.03,.98,.44,.29,.8,gold,fly);for(let i=0;i<5;i++){const z=.53+i*.22;const ring=mesh(new THREE.TorusGeometry(.32-i*.018,.014,6,36),darkgold,0,1.045,z,fly);ring.scale.x=1.23;ring.scale.y=.78}
orb(0,1.08,.13,.33,.28,.38,gold,fly);orb(0,1.17,-.37,.3,.27,.3,gold,fly);orb(-.23,1.25,-.47,.13,.17,.17,eye,fly);orb(.23,1.25,-.47,.13,.17,.17,eye,fly);
...
function frame(now){const dt=Math.min((now-last)/1000||0,.05);last=now;clock+=dt;motion.step(engine,dt);bar.position.set(engine.running&&engine.stall>0?Math.sin(clock*45)*.01:0,motion.y,motion.z);rackAssist.visible=motion.assisted;rackAssist.position.y=motion.y-BENCH.barRadius-.04;rackAssist.position.z=motion.z;
 for(const r of armRig){const shoulder=[r.s*.23,1.12,-.05],hand=[r.s*.79+bar.position.x,motion.y,motion.z],elbow=elbowFor(shoulder,hand,r.s);moveRod(r.upper,shoulder,elbow,r.upperLength);moveRod(r.lower,elbow,hand,r.lowerLength);r.hand.position.set(...hand)}
 ...
 sample+=dt;if(sample>.06){sample=0;history.push(engine.effort*(.7+.25*Math.sin(clock*22))+(engine.running?Math.sin(clock*73)*9:Math.sin(clock*5)*2));history.shift();drawTrace(clock)}brainPanel.update(dt);renderer.render(scene,camera);requestAnimationFrame(frame)}requestAnimationFrame(frame);
