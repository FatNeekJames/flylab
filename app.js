...
const brainPanel=new BrainPanel(engine),trace=$('trace').getContext('2d');const history=new Array(160).fill(0);let last=0,clock=0,sample=0,version=-1,prevRep=0,savedManualWeight=null,workoutKey=null;
...
function frame(now){const dt=Math.min((now-last)/1000||0,.05);last=now;clock+=dt;if(!life.active&&life.owner)motion.step(engine,dt);
 const blackjackState=blackjack.sync(),activity=life.skill,pose=activityView.update(activity,clock,blackjackState,core.current),growth=core.development.growth;sceneDistance=activityView.cameraDistance;sceneHeight=activityView.cameraHeight;view();floor.scale.set(pose?1.5:1,1,pose?1.5:1);host.setAttribute('aria-label',pose?`${activity.label}: ${activityView.status}. Drag to orbit; scroll to zoom.`:'Interactive 3D fly bench press. Drag to orbit; scroll to zoom.');thorax.scale.set(.33*(1+.16*growth),.28*(1+.08*growth),.38);abdomen.scale.x=.44*(1+.06*growth);
 const autoBench=life.active&&activity?.presentation.prop==='barbell',plan=core.current?.plan,training=plan?workoutProgress(plan,core.current.elapsed):null;
 if(autoBench&&plan?.kind==='strength'){if(savedManualWeight===null)savedManualWeight=engine.weight;if(workoutKey!==core.current.uid){workoutKey=core.current.uid;load(plan.weight)}}else if(savedManualWeight!==null){load(savedManualWeight);savedManualWeight=null;workoutKey=null;}
 let barY=motion.y,barZ=motion.z;
 if(autoBench){barY=BENCH.rack;barZ=BENCH.rackZ;if(training?.phase==='lifting'){barY=BENCH.bottom+training.position*(BENCH.lockout-BENCH.bottom);barZ=BENCH.liftZ;}else if(['unrack','rack'].includes(training?.phase)){const u=training.phase==='unrack'?training.transition:1-training.transition;if(u<.5){barY=BENCH.rack+(BENCH.lockout-BENCH.rack)*u*2;}else{barY=BENCH.lockout;barZ=BENCH.rackZ+(BENCH.liftZ-BENCH.rackZ)*(u-.5)*2;}}}
 ...
 if(training&&autoBench){$('reps').textContent=training.reps;$('state').textContent=training.phase.toUpperCase();$('callout').textContent=training.label;}
 ...
 $('stageLoadLabel').textContent=pose?'SESSION':'ON THE BAR';$('stageRepLabel').textContent=pose?'MASTERY %':autoBench?'WORKOUT REPS':'COMPLETED REPS';$('stageBestLabel').textContent=pose?'SESSIONS':'PERSONAL BEST';$('stageLoadUnit').textContent=pose?'%':'LB';$('stageBestUnit').textContent=pose?'':'LB';
