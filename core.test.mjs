...
test('wall-time tick and catch-up are deterministic across reloads',()=>{const a=core();a.setMode('auto');a.wallTick(1000);a.wallTick(12000);const b=core();assert.ok(b.restore(a.serialize()));a.wallTick(91000);b.wallTick(91000);assert.equal(a.serialize(),b.serialize());assert.equal(b.completed,0);assert.equal(b.time,90)});
