export class FlyEngine {
 ...
 if(this.phase>=1){this.reps++;this.best=Math.max(this.best,this.weight);this.phase=0;this.position=1;this.event=`${this.weight} lb locked out. ${this.single?'One-rep attempt complete!':'Clean rep. Keep going.'}`;this.version++;if(this.single){this.running=false;this.resting=true}}
 }
}
