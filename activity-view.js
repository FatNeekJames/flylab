...
import {trackPath,poolPath,stairPath,stairHeight,TAU} from './activity-motion.mjs';
...
export class ActivityView {
 ...
 room(prop){
  ...
  if(prop==='track'){
   ...
   for(const x of [-1.375,1.375])for(let i=0;i<8;i++){box(0x42617c,[x,(i+1)*.10,2.475-i*.65],[1.3,(i+1)*.20,.65]);box(0xe3c96f,[x,(i+1)*.20+.008,2.77-i*.65],[1.3,.018,.045]);}
   box(0x5b788d,[0,.8,-3.0],[4.4,1.6,1.8]);
   for(const x of [-2.13,2.13]){for(let i=0;i<5;i++)box(0x96b1b7,[x,.5+i*.32,2.5-i*1.3],[.055,1,.055]);this.line(g,[[x,1,2.8],[x,2.6,-2.5],[x,2.6,-3.3]],0xa9c6ca,.035);}this.label(g,'UP   /   DOWN',[0,2.7,-3.25],2.5);
 ...
 update(skill,clock){
  ...
  const feet=(side,index)=>{
   ...
   if(running||stairs){
    ...
    target.y+=(stairs?stairHeight(target.z):.08);return local(target);
   }
  ...
  this.status=running?`Lap ${Math.floor(t*.30/TAU)+1} · oval circuit`:swim?`Length ${Math.floor(t*.8/(6+Math.PI*.55))+1} · lane turns`:stairs?`${Math.round(path.y/.2)} / 8 steps · walking circuit`:prop==='dumbbells'?'Standing cable fly':prop==='kitchen'?'Stir · chop · repeat':prop==='books'?'Read · type · revise':sleep?'Sleep & recover':prop==='mat'?'Standing stretch & balance':'Table practice';
