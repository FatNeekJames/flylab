...
Blank research topics now trigger open-ended web discovery rather than a fixed topic list. The first checkpoint searches for one new subject, considers the last 30 selected subjects to encourage variety, and records its chosen title in the journal. Remaining checkpoints stay on that subject for the research hour. A custom topic or question remains unrestricted text. Discovery shares the existing checkpoint budget and API-key requirements; it adds no separate API call. Previously saved sessions are preserved.
