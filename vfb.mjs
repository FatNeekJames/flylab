...
export class VFBClient{
 constructor({fetcher=globalThis.fetch,now=()=>Date.now(),ttl=86400000}={}){this.fetcher=fetcher;this.now=now;this.ttl=ttl;this.cache=new Map();this.pending=new Map()}
 async request(path){
  ...
  const request=(async()=>{const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),15000);
   try{const response=await this.fetcher(VFB_BASE+path,{signal:controller.signal,credentials:'omit'});if(!response.ok)throw new Error('VFB is temporarily unavailable');const data=await response.json();if(this.cache.size>=64)this.cache.delete(this.cache.keys().next().value);this.cache.set(path,{data,at:this.now()});return data}finally{clearTimeout(timeout);this.pending.delete(path)}
 ...
 async term(id){recordURL(id);const path='/get_term_info?'+new URLSearchParams({id});const data=await this.request(path);return normalizeTerm(data,id,this.cache.get(path)?.at??this.now())}
}
