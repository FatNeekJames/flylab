...
## Study and Minecraft implementation — 21 September 2026

The study service follows the official OpenAI [web search](https://developers.openai.com/api/docs/guides/tools-web-search) and [structured output](https://developers.openai.com/api/docs/guides/structured-outputs) APIs: search evidence first, then structure notes linked to the returned source list. The local note learner is an online softmax classifier with hashed word features and replay; its held-out question scores only measure retrieval of the corresponding stored note. Stable topic/concept identifiers let repeated study reuse existing learned classes. It does not improve the language model's weights or certify the accuracy of generated explanations.

[Voyager](https://voyager.minedojo.org/) demonstrates a Minecraft agent using a curriculum, persistent skills and environment feedback without fine-tuning the language model. FlyLab adopts the small-step curriculum and outcome recording approach, not Voyager's performance claims or its general code-generation agent. The initial Minecraft connector uses the documented [Mineflayer](https://github.com/PrismarineJS/mineflayer) and [pathfinder](https://github.com/PrismarineJS/mineflayer-pathfinder) APIs. It has three built-in starter actions and a UCB target selector that updates from successful inventory changes. It does not execute downloaded or AI-generated code. This implementation still requires live verification against the user's Java LAN world.

The first live OpenAI probe reached the API but returned `credit_balance_exhausted` / `insufficient_quota` (HTTP 429). No successful AI-generated research is claimed from that probe. Tests use explicit fixtures, including provider failures, and verify persistence, source validation, cancellation and learned recall separately from external availability.
