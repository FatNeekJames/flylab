...
export class ActivityCore {
  ...
  reset() {
    this.stats = { energy: 100, fatigue: 0 }; this.workouts = {};
  ...
  context(id) { return { ...this.states[id], workouts: this.workouts[id], stats: { ...this.stats }, modifiers: this.modifiers() }; }
  ranking() {
    return [...this.skills].map(([id, skill]) => {
      ...
      const plan=skill.createWorkout?.(state);
      const cost = skill.effortCost(state)*(plan?.multiplier??1), baseEffects = skill.sessionEffects?.(state) ?? { energy: -cost, fatigue: cost * .5 };
      const effects=plan?{energy:baseEffects.energy*(plan.multiplier),fatigue:baseEffects.fatigue*plan.multiplier}:baseEffects;
      ...
      return { id, label: skill.label, need, eligible, score: eligible ? need + (id === this.nudge ? 1 : 0) : -1,
        reason: eligible ? (id === this.nudge ? 'Your next-session nudge · ' : '') + (skill.explain?.(state, hours) ?? 'Highest current need') : 'Needs more energy first', cost, effects, plan };
  ...
  start(id) {
    ...
    const duration = entry.plan?.duration ?? skill.sessionDuration?.(context) ?? skill.duration ?? 1200;
    if (!Number.isFinite(duration) || duration <= 0) throw new RangeError('Invalid session duration');
    this.selected = id;
    this.current = { uid: crypto.randomUUID(), id, elapsed: 0, duration, plan: entry.plan??null, effects: entry.effects, reason: entry.reason, start: this.time };
  ...
  advance(seconds) {
    ...
    while (remaining > 1e-7) {
      ...
      if (session.elapsed >= session.duration - 1e-7) {
        ...
        if(session.plan){const p=this.workouts[session.id]??{completed:0,progress:0};this.workouts[session.id]={completed:p.completed+1,progress:p.progress+(session.plan.deload?0:1)};}
        this.history.unshift({ uid: session.uid, id: session.id, at: this.time, gain: this.lastGain, plan: session.plan??null, reason: session.reason });
  ...
  serialize() {
    return JSON.stringify({ version: SAVE_VERSION, workouts: this.workouts, stats: this.stats, states: this.states, time: this.time, mode: this.mode,
  ...
  restore(raw) {
    try {
      ...
      for(const id of this.skills.keys()){const p=value.workouts?.[id];if(p&&Number.isInteger(p.completed)&&Number.isInteger(p.progress)&&p.completed>=0&&p.progress>=0&&p.progress<=p.completed)this.workouts[id]={completed:Math.min(p.completed,100000),progress:Math.min(p.progress,100000)};}
      ...
      if (c && this.skills.has(c.id) && Number.isFinite(c.duration) && c.duration > 0 && c.duration <= 86400 && Number.isFinite(c.elapsed) && c.elapsed >= 0 && c.elapsed < c.duration && Number.isFinite(c.effects?.energy) && Number.isFinite(c.effects?.fatigue)) {
        this.current = { uid: typeof c.uid==='string'?c.uid:crypto.randomUUID(), id: c.id, elapsed: c.elapsed, duration: c.duration, plan: null, effects: { energy: clamp(c.effects.energy, -100, 100), fatigue: clamp(c.effects.fatigue, -100, 100) }, reason: typeof c.reason === 'string' ? c.reason.slice(0, 200) : 'Resumed session', start: this.time - c.elapsed };
        const planned=this.skills.get(c.id).createWorkout?.(this.context(c.id));
        if(c.plan&&planned){const p=c.plan;if(['strength','endurance'].includes(p.kind)&&Number.isFinite(p.duration)&&p.duration===c.duration&&p.duration>0&&p.duration<=86400&&Number.isFinite(p.multiplier)&&p.multiplier>0&&p.multiplier<=2&&((p.kind==='strength'&&Number.isInteger(p.sets)&&p.sets>=1&&p.sets<=5&&Number.isInteger(p.reps)&&p.reps>0&&p.reps<=12&&Number.isFinite(p.weight)&&p.weight>0&&p.weight<=400&&p.repSeconds===5&&p.warmup===180&&[120,180].includes(p.rest))||(p.kind==='endurance'&&Number.isFinite(p.target)&&p.target>0&&p.target<=6000&&Number.isFinite(p.pace)&&p.pace>0&&p.pace<=90)))this.current.plan={...p};}
